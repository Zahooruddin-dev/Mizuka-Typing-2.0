...
const TypeBox = ({
  ...
  useEffect(() => {
    if (currWordIndex === DEFAULT_WORDS_COUNT - 1) {
      ...
      // multiple if statements are so that if in future I want to add a new language I could do so.
    }
              ...
              >
                <Tooltip
                  title={
                    language === ENGLISH_MODE
                      ? HARD_DIFFICULTY_TOOLTIP_TITLE
                      : HARD_DIFFICULTY_TOOLTIP_TITLE /// if another language is added this code should support it 
                  }
                >
                  <span
